    ...
    async fn enumerate(&self, domain: &str) -> Result<Vec<String>, Error> {
        ...
        let subdomains: HashSet<String> = web_archive_urls
            ...
            .filter_map(|url| {
                Url::parse(&url)
                    .map_err(|err| {
                        log::error!("{}: err parsing url: {}", self.name(), err);
                        err
                    })
                    .ok()
            })
